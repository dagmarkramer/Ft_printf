...
/*   Updated: 2021/01/21 12:00:30 by dkramer       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_formatchecker(const char *format, t_remember *z)
{
	if (format[z->i] != '-' && !(format[z->i] >= '0' &&
			format[z->i] <= '9') && format[z->i] != '*' &&
				format[z->i] != '.' && format[z->i] != 'c' &&
					format[z->i] != 's' && format[z->i] != 'p' &&
						format[z->i] != 'i' && format[z->i] != 'u'
							&& format[z->i] != 'd' && format[z->i] != 'x'
